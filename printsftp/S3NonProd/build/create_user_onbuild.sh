...
pull_templates()
{
   ...
   /usr/local/bin/aws s3 cp s3://${s3bucket}/build/passwd.gpg /home/control/build/passwd.gpg
   /usr/local/bin/aws s3 cp s3://${s3bucket}/build/group.gpg /home/control/build/group.gpg
...
create_dirandfiles()
{
  ...
  do
    ...
    if [ ! -z "$pwline" ] ; then
        ...
        /sbin/useradd --no-create-home --uid $userid --gid 10000 --home-dir $homedir --comment "print-sftp-user" --shell /sbin/nologin $username
   fi
